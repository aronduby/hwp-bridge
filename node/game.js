 ...
 * @property {Record<string, PlayerStats>} stats - dictionary of player stats with key of name_key
 ...
 * @property {array<array<Record<string, int>>>} boxscore - array representing box scores, 0 is us, 1 is opponent, next level is quarters which contain nameKeys string and goal count val
 * @property {array<int>} score - current score, 0 is us, 1 is opponent
 */

/**
 * # !!IMPORTANT!! You Probably Don't Want This!
 * If you are using this class directly outside the tests, then you are probably doing something wrong. You should be
 * getting your games from the GameFactory instead.
 *
 * @param game_id
 * @param emitter
 ...
 * @constructor
 */
function Game(game_id, emitter, gameData) {
    this.game_id = this.data.game_id = game_id;
    this.loaded = false;
    this.emitter = emitter;

    this.data = { ...this.data, ...gameData };

    Object.values(this.data.stats).forEach((p) => this._setPlayerNumber(p));
}
...
Game.prototype = {
    ...
    // region Non Data Affecting Methods
    ...
    /**
     * Used the other_numbers and game data to set the number property for the player
     * @param player
     * @private
     */
    _setPlayerNumber: function (player) {
        if (this.data.team && player.other_numbers && player.other_numbers[this.data.team]) {
            player.number = player.other_numbers[this.data.team];
            player.number_sort = parseInt(player.number, 10);
        }
    }
    // endregion
};
