...
class Changelog extends Renderer {
	...
	public function setup(){
		$this->title = 'New Changes Made';
		
		$changelog_id = json_decode($this->content);
		$stmt = $this->dbh->query("SELECT changes FROM changelog WHERE change_id = ".intval($changelog_id));
		$changes = $stmt->fetch(\PDO::FETCH_COLUMN);
		$this->content = $this->excerptAndHighlight($changes, null, 150);
